...
class cube():
    ...
    def read_cube(self, fname):
        ...
        with open(fname, 'r') as fin:
            ...
            self.Y = np.array([float(nVoxel[1]),
                               float(nVoxel[2]),
                               float(nVoxel[3])])
            nVoxel = fin.readline().split()
            ...
            for atom in range(self.natoms):
                ...
                self.atomsXYZ.append(list(map(float,
    ...
    def rotate_cube(self, angle, axes=None):
        '''
        Rotate cube data around a plane. The plane is defined in the axes
        variable with origin set as point. For example, to rotate along the
        xy plane, axes would be defined as (0,1).
        '''
        if 0 not in axes:
            rotAxis = 'x'
        elif 1 not in axes:
        ...
        # Centre of new cell
        centreNewCell = np.sum(r.apply([self.X * self.NX,
                                        self.Y * self.NY,
                                        self.Z * self.NZ]) / 2., axis=0)
    ...
    def planar_average(self, axis):
        '''
        Calculate the planar average along an axis. The axis is given as a
        string of either x,y or z.
        '''
    ...
    def cube_int_atom(self, atomID, radius):
        '''
        Integrate the cube data in a sphere around a particular atom. Needs
        the atom number (note that atom 0 is the first atom). Also needs a
    ...
    def super_cube(self, new_size):
        ...
        new_data = np.zeros([new_size[0] * self.NX,
                             new_size[1] * self.NY,
                             new_size[2] * self.NZ])
        ...
        for x in range(new_size[0]):
            for y in range(new_size[1]):
                for z in range(new_size[2]):
                    new_data[x * self.NX:(x + 1) * self.NX,
...
def expand_cell(files, new_size):
    ...
    cube_in.write_cube('expand_%dx%dx%d.cube' % (new_size[0],
                                                 new_size[1],
                                                 new_size[2]))
    return None
...
#    Take the planar average")
    ...
    parser.add_argument("-s", "--subtract",
                        help="Subtract two or more cube files together",
                        action="store_true")
    parser.add_argument("-M", "--multiply",
                        help="Multiply two or more cube files together",
                        action="store_true")
    parser.add_argument("-p", "--power",
                        help="Raise the cube file to a certain power. Any number of cube files can be specified and they will all be raised to the power defined. Default is to square the cube file(s).",
                        nargs='?', const=2, type=int)
    parser.add_argument("-i", "--integrate",
                        help="Integrate over the entire cube file.",
                        action="store_true")
    parser.add_argument("-ia", "--integrateatom",
                        help="Integrate a sphere around a particular atom. Needs atom id and a radius (in Angstrom) to integrate within.",
                        nargs=2)
    parser.add_argument("-ir", "--integrateref",
                        help="Integrate a sphere around a reference xyz (given in Angstrom). Supply x y z r, where r is the radius (in Angstrom).",
                        nargs=4)
    parser.add_argument("-e", "--expand",
                        help="Make a supercell of the specified cube file",
                        nargs=3, type=float)
    parser.add_argument("-m", "--mean",
                        help="Calculate planar average of a cube file along a particular axis. Normalized to give total number of electrons. Arguments are x,y or z.",
                        nargs=1, type=str)
    parser.add_argument("-t", "--translate",
                        help="Translate a cube file. Requires a translation vector as an argument.",
                        nargs=3, type=float)
    parser.add_argument("-r", "--rotate",
                        help="Rotate a cube file. Requires an angle and an axis around which to rotate as an argument. The axis is provided with the -ax flag.",
                        nargs=1, type=float)
    ...
    if args.integrateatom:
        if args.Files:
            cube_integrate_atom(args.Files,
                                args.integrateatom[0], args.integrateatom[1])
